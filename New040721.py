...
- Dynamic split NEW vs RETX (M_MAX=44)
...
# ============================
# Core Engine with Dynamic Reservation + PI control (+ 2-choice + controlled overflow)
# ============================

def actualTrafficPattern(
    arrivals_per_group, burst_mask, frameSize=0.005, backoffBool=True,
    PERSIST_K_GEN=0.48,         # base for general pools (slightly lower)
    PERSIST_K_RES=1.20,         # base for reserved pools (slightly higher)
    TARGET_FILL=0.78,           # reserved utilization target (<1)
    ...
    OVERFLOW_P=0.35,            # controlled overflow prob. for burst UEs to general
    RESERVATION=None, CARVE_OUT=True,
    RES_POLICY=None
):
    M_MAX = 54
    preambleTransMax = 10  # standards cap
